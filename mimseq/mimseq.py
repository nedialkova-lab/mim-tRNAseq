...
def mimseq(trnas, trnaout, name, species, out, cluster, cluster_id, cov_diff, posttrans, control_cond, threads, max_multi, snp_tolerance, \
	keep_temp, cca, double_cca, min_cov, mismatches, remap, remap_mismatches, misinc_thresh, mito_trnas, plant, pretrnas, local_mod, p_adj, sample_data):
...
# Main wrapper
	...
	= modsToSNPIndex(trnas, trnaout, mito_trnas, plant, modifications, name, out, double_cca, threads, snp_tolerance, cluster, cluster_id, posttrans, pretrnas, local_mod)
	structureParser()
...
def main():
	...
	inputs.add_argument('-s','--species', metavar='species', required = not ('-t' in sys.argv), dest = 'species', help = \
		'Species being analyzed for which to load pre-packaged data files (prioritized over -t, -o and -m). Options are: Hsap, Hsap19, Mmus, Rnor, Scer, ScerMut, Spom, Dmel, Drer, Ecol', \
	...
	inputs.add_argument('-m', '--mito-trnas', metavar = 'mitochondrial/plastid tRNAs', required = False, dest = 'mito', \
		help = 'Mitochondrial/plastid tRNA fasta file. Should be downloaded from mitotRNAdb for species of interest. Already available in data folder for a few model organisms.')
	...
	parser.set_defaults(threads=1, out="./", max_multi = 3, mito = '', plant = False, cov_diff = 0.5)
		...
		if not args.species and not (args.trnas or args.trnaout):
			...
			if args.species:
				...
				if args.species == 'Atha':
					...
					args.mito = os.path.dirname(os.path.realpath(__file__)) + "/data/araTha1-eColitK/araTha1-plastidtRNAs.fa"
					args.plant = True
			else:
			...
			mimseq(args.trnas, args.trnaout, args.name, args.species, args.out, args.cluster, args.cluster_id, args.cov_diff, \
				...
				args.misinc_thresh, args.mito, args.plant, args.pretrnas, args.local_mod, args.p_adj, args.sampledata)
