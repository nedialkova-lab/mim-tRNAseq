...
if (length(args)==0) {
	...
	cov_byaa_norm = ggplot(cov_byaa, aes(x = bin, y = cov_norm, fill = aa, group = aa)) + geom_bar(stat = "identity") + facet_wrap(~bam, ncol = 4) + 
	...
	cov_byaa_norm_scaled = ggplot(cov_byaa_scaled, aes(x = bin, y = cov_norm_scaled, fill = aa, group = aa)) + geom_bar(stat = "identity") + facet_wrap(~bam, ncol = 4) + 
	...
	plot_func = ggplot(cov_bygene, aes(x = bin, y = cov_norm)) + geom_bar(stat = 'identity') + 
  	...
  	ggsave(paste(out_dir, "coverage_bycluster_norm.pdf", sep = ''), multi, height = 12, width = 8)
